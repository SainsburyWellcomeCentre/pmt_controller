...
class MicroFont:
    ...
    def write(self, txt, fb_fmt, fb_width, fb_height, x, y, color, *, rot=0, x_spacing=0, y_spacing=0):
        off_x = 0
        off_y = 0
        tmp_buf = bytearray(fb_width*fb_height*2)
        for c in txt:
            if c == "\n":
                ...
                continue
            ch = self.get_ch(c)
            self.draw_ch(ch, tmp_buf, fb_fmt, fb_width, fb_height, x, y, color, off_x, off_y, rot)
